...
class RelPositionMultiHeadAttention(MultiHeadAttention):
    ...
    @torch.jit.script
    def slice_helper_matrixb_bd(matrix_bd, matrix_ac):
        return matrix_bd[:, :, :, : matrix_ac.size(-1)]
...
class RelPositionalEncoding(PositionalEncoding):
    ...
    @torch.jit.script
    def slice_helper_pos_emb(x, pe):
        center_pos = pe.size(1) // 2
        start_pos = center_pos - x.size(1) + 1
        end_pos = center_pos + x.size(1)
        return pe[:, start_pos:end_pos]
